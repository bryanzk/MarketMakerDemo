...
class TestHyperliquidPricesEndpoint:
    ...
    def test_prices_endpoint_missing_symbols_param(self, test_client):
    ...
    def test_prices_endpoint_empty_symbols(self, test_client):
    ...
    def test_prices_endpoint_exchange_not_initialized(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_prices_endpoint_single_symbol_success(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_prices_endpoint_multiple_symbols_success(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    @patch("server._hyperliquid_price_cache")
    def test_prices_endpoint_symbol_fetch_error(self, mock_cache, mock_get_exchange, test_client):
        """Test prices endpoint handles symbol fetch errors gracefully / 测试端点优雅处理交易对获取错误"""
        # Clear cache to ensure we test error handling without cached values
        # 清除缓存以确保在没有缓存值的情况下测试错误处理
        mock_cache.clear()
        ...
        assert response.status_code == 200
        data = response.json()
        # Should still return prices dict with None for failed symbol (when no cache)
        # 应该仍然返回价格字典，失败的交易对为 None（当没有缓存时）
        assert "prices" in data
        # When fetch fails and no cache exists, price should be None
        # 当获取失败且没有缓存时，价格应该为 None
...
class TestHyperliquidConnectionEndpoint:
    ...
    @patch("server.get_exchange_by_name")
    def test_connection_endpoint_exchange_not_initialized(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_connection_endpoint_not_connected(self, mock_get_exchange, test_client):
    ...
    def test_connection_endpoint_connected_with_fresh_data(
        self, mock_time, mock_get_exchange, test_client
    ):
    ...
    @patch("server.get_exchange_by_name")
    @patch("time.time")
    def test_connection_endpoint_stale_market_data(self, mock_time, mock_get_exchange, test_client):
...
class TestHyperliquidCancelOrderEndpoint:
    ...
    @patch("server.get_exchange_by_name")
    def test_cancel_order_endpoint_exchange_not_connected(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_cancel_order_endpoint_exchange_not_initialized(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_cancel_order_endpoint_success(self, mock_get_exchange, test_client):
    ...
    @patch("server.get_exchange_by_name")
    def test_cancel_order_endpoint_handles_exception(self, mock_get_exchange, test_client):
...
class TestHyperliquidBotControl:
    ...
    def test_control_start_with_hyperliquid_instance(self, mock_get_default_exchange, mock_bot_engine, test_client):
        """
        ...
        with patch("server.threading.Thread") as mock_thread_class:
            ...
            response = test_client.post("/api/control?action=start")
    ...
    def test_control_start_without_strategy_instance(self, mock_get_default_exchange, mock_bot_engine, test_client):
        """
    ...
    @patch("server.bot_engine")
    @patch("server.get_default_exchange")
    def test_control_start_with_invalid_spread(self, mock_get_default_exchange, mock_bot_engine, test_client):
        """
    ...
    @patch("server.bot_engine")
    def test_control_stop(self, mock_bot_engine, test_client):
        """
        Test stopping bot
        测试停止 bot
        """
        ...
        response = test_client.post("/api/control?action=stop")
    ...
    @patch("server.bot_engine")
    def test_control_invalid_action(self, mock_bot_engine, test_client):
        """
        Test control endpoint with invalid action
        测试使用无效操作的控制端点
        """
        response = test_client.post("/api/control?action=invalid")
    ...
    @patch("server.bot_engine")
    @patch("server.is_running", False)
    def test_status_endpoint_returns_active_field(self, mock_bot_engine, test_client):
        """
