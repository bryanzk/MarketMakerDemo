...
*   **Coverage**: Aim for 100% coverage on core modules (`src.ai.agents`, `src.trading.strategies`, `src.trading`).
    *   **覆盖率**：核心模块（`src.ai.agents`、`src.trading.strategies`、`src.trading`）的目标覆盖率 100%。
