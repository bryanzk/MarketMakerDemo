...
class TestHyperliquidClientRateLimitRetry:
    ...
    def test_rate_limit_retry_fails_on_second_attempt(self, mock_sleep, mock_post):
        ...
        # Set up mock to return 429 on both attempts
        # 设置 mock 在两次尝试中都返回 429
        mock_post.side_effect = [
            mock_rate_limit_response,  # First call: 429
            mock_retry_failure,  # Retry: still 429
        ]
        ...
        # Verify sleep was called with Retry-After value (10 seconds)
        # 验证 sleep 被调用，使用 Retry-After 值（10 秒）
        mock_sleep.assert_called_once_with(10)

        # Verify result is None after retry failure
        # 验证重试失败后结果为 None
