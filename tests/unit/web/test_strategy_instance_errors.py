...
class TestStrategyInstanceErrorsExposure:
    ...
    def test_status_endpoint_includes_errors_field(self, test_client):
        """
        ...
        """
        # Mock bot_engine.get_status() to return a proper dict / Mock bot_engine.get_status() 返回正确的字典
        with patch("server.bot_engine.get_status") as mock_get_status:
            ...
            with patch("server.is_running", False):
    ...
    def test_errors_field_has_global_alert(self, test_client):
        """
        ...
        # Mock bot_engine.get_status() and set alert / Mock bot_engine.get_status() 并设置 alert
        with patch("server.bot_engine.get_status") as mock_get_status:
            mock_get_status.return_value = {
                "symbol": "ETH/USDC:USDC",
                "spread": 0.001,
            ...
            with patch("server.bot_engine.alert", test_alert):
                with patch("server.is_running", False):
                    response = test_client.get("/api/status")
    ...
    def test_errors_field_has_global_error_history(self, test_client):
        """
        ...
        test_error_history = deque(test_errors, maxlen=200)
        
        # Mock bot_engine.get_status() and error_history / Mock bot_engine.get_status() 和 error_history
        with patch("server.bot_engine.get_status") as mock_get_status:
            mock_get_status.return_value = {
                "symbol": "ETH/USDC:USDC",
                "spread": 0.001,
            ...
            with patch("server.bot_engine.error_history", test_error_history):
                with patch("server.is_running", False):
                    response = test_client.get("/api/status")
    ...
    def test_errors_field_has_instance_errors(self, test_client):
        """
        ...
        """
        # Mock bot_engine.get_status() and strategy_instances / Mock bot_engine.get_status() 和 strategy_instances
        with patch("server.bot_engine.get_status") as mock_get_status:
            mock_get_status.return_value = {
                "symbol": "ETH/USDC:USDC",
                "spread": 0.001,
            ...
            with patch("server.bot_engine.strategy_instances", {}):
                with patch("server.is_running", False):
                    response = test_client.get("/api/status")
    ...
    def test_instance_errors_include_alert_and_history(self, test_client):
        """
        ...
        """
        # Create mock instance with alert and error history / 创建带有 alert 和 error_history 的 mock 实例
        test_alert = {
            "type": "error",
        ...
        mock_instance.alert = test_alert
        mock_instance.error_history = deque(test_errors, maxlen=200)
        mock_instance.symbol = "ETH/USDC:USDC"
        
        mock_strategy_instances = {"default": mock_instance}
        ...
        with patch("server.bot_engine.get_status") as mock_get_status:
            ...
            with patch("server.bot_engine.strategy_instances", mock_strategy_instances):
                with patch("server.is_running", False):
                    response = test_client.get("/api/status")
    ...
    def test_error_history_includes_trace_id(self, test_client):
        """
        ...
        test_error_history = deque([test_error], maxlen=200)
    ...
    def test_error_history_limit_enforced(self, test_client):
        """
    ...
    def test_instance_error_history_limit_enforced(self, test_client):
        """
        ...
        Then: Only last 20 entries should be returned
        """
        # Create mock instance with 25 errors / 创建带有 25 个错误的 mock 实例
        test_errors = [
            {
    ...
    def test_errors_field_structure(self, test_client):
        """
        ...
        """
        # Mock bot_engine.get_status() / Mock bot_engine.get_status()
        with patch("server.bot_engine.get_status") as mock_get_status:
            mock_get_status.return_value = {
                "symbol": "ETH/USDC:USDC",
                "spread": 0.001,
    ...
    def test_status_response_includes_trace_id(self, test_client):
        """
    ...
    def test_multiple_instances_errors_exposed(self, test_client):
        """
        ...
        """
        # Create mock instances / 创建 mock 实例
        test_instance_id = "test_instance"
        
        mock_instance1 = MagicMock()
        mock_instance1.alert = None
        mock_instance1.error_history = deque(maxlen=200)
        mock_instance1.symbol = "ETH/USDC:USDC"
        
        mock_instance2 = MagicMock()
        mock_instance2.alert = {
            "type": "warning",
            "message": "Test instance alert"
        }
        mock_instance2.error_history = deque([{
            "timestamp": time.time(),
            "type": "test_error",
            "message": "Test instance error",
            "trace_id": "test_trace"
        }], maxlen=200)
        mock_instance2.symbol = "BTC/USDC:USDC"
        
        mock_strategy_instances = {
            "default": mock_instance1,
            test_instance_id: mock_instance2,
        }
        ...
        # Should have at least one instance / 应该至少有一个实例
